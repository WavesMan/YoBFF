...
type VersionHistoryTabProps = {
  ...
  handleDelete: (versionId: string) => Promise<void>
}

export function VersionHistoryTab({ versions, handleRollback, handleDelete }: VersionHistoryTabProps) {
  return (
    <div className="panel">
      <h4>历史版本</h4>
        ...
        {versions.map(v => (
          <div key={v.id} className="version-item">
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="button small" onClick={() => handleRollback(v.id)}>
                回滚
              </button>
              <button className="button small danger" onClick={() => handleDelete(v.id)}>
                删除
              </button>
            </div>
          </div>
